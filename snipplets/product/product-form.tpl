...
{% endembed %}
    {% if product.brand %}<div class="product-topdescription">{{ product.brand }}</div>{% endif %}

{# Product price #}

<div class="price-container text-center text-md-left" data-store="product-price-{{ product.id }}">
    <span class="d-inline-block">
	   <h4 id="compare_price_display" class="js-compare-price-display price-compare {% if product_can_show_installments or (product.promotional_offer and not product.promotional_offer.script.is_percentage_off) %}mb-2{% endif %}" {% if not product.compare_at_price or not product.display_price %}style="display:none;"{% else %} style="display:block;"{% endif %}>{% if product.compare_at_price and product.display_price %}<span class="dinheiro">R$</span>{{ product.compare_at_price // 100 }}{% endif %}</h4>
    </span>
    <span class="d-inline-block">
    	<h4 class="js-price-display {% if product_can_show_installments or (product.promotional_offer and not product.promotional_offer.script.is_percentage_off) %}mb-2{% endif %}" id="price_display" {% if not product.display_price %}style="display:none;"{% endif %}>{% if product.display_price %}<span class="dinheiro">R$</span>{{ product.price // 100 }}{% endif %}</h4>
    </span>
    {# Product installments #}

    <span class="d-inline-block prestaciones">
...
{# Product form, includes: Variants, CTA and Shipping calculator #}
 ...
 <form id="product_form" class="js-product-form" method="post" action="{{ store.cart_url }}" data-store="product-form-{{ product.id }}">
    ...
    <div class="compra-produto">
        ...
        <input type="submit" class="js-addtocart js-prod-submit-form btn btn-primary btn-block {{ state }}" value="comprar recordação" {% if state == 'nostock' %}disabled{% endif %} />
